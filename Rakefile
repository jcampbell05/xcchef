...
def generate_docs
  ...
  sh "jazzy --clean --sdk macosx --xcodebuild-arguments -scheme,xcchef --skip-undocumented --no-download-badge"
end
...
def get_archive_url(version)
  'https://github.com/xcodeswift/xcchef/archive/' + "#{version}" + '.tar.gz'
end

def download_archive(url)
  # follow link, output errors, no progress meter
  `curl -LSs #{url} -o xcchef.tar.gz`
end

def get_checksum
  `shasum -a 256 xcchef.tar.gz | awk '{printf $1}'`
end
...
desc "Bumps the version of xcchef. It creates a new tagged commit and archives the binary to be published with the release"
